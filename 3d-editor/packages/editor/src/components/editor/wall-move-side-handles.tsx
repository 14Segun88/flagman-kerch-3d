...
function createEmptyGeometry(): BufferGeometry {
  const g = new BufferGeometry()
  g.setAttribute('position', new Float32BufferAttribute([], 3))
  g.setAttribute('uv', new Float32BufferAttribute([], 2))
  return g
}

function buildDashedVerticalGeometry(height: number) {
  if (!(Number.isFinite(height) && height > 0)) return createEmptyGeometry()
  ...
  const merged =
    dashes.length > 0
      ? (mergeGeometries(dashes, false) ?? createEmptyGeometry())
      : createEmptyGeometry()
