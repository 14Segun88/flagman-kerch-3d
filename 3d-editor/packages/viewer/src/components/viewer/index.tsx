...
function GPUDeviceWatcher() {
  ...
  useEffect(() => {
    ...
    // best). Pipe non-transient errors to console.error so silent mobile crashes show up.
    const onUncapturedError = (event: any) => {
      const msg = event?.error?.message || String(event?.error || '')
      if (msg.includes('Vertex buffer slot') || msg.includes('Invalid CommandBuffer')) {
        console.warn('[viewer] WebGPU transient validation warning:', msg)
        return
      }
      console.error('[viewer] WebGPU uncaptured error:', msg, event?.error)
    }
