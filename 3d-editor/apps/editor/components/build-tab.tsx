...
export function BuildTab() {
  ...
  const openNimModal = useNimModal((s) => s.openModal)
  ...
  return (
    <div className="flex h-full flex-col gap-3 p-3">
      {/* DeepFloorPlan & FloorPlan3D Studio Banner */}
      <button
        onClick={openNimModal}
        type="button"
        className="w-full flex items-center justify-between gap-2 rounded-xl border border-emerald-500/40 bg-emerald-500/10 p-2.5 text-left text-xs font-bold text-emerald-400 hover:bg-emerald-500/20 transition cursor-pointer shadow-sm"
      >
        <span className="flex items-center gap-2">
          <Sparkles className="h-4 w-4 shrink-0 text-emerald-400 animate-pulse" />
          <span>DeepFloorPlan 2D➔3D & Планировки</span>
        </span>
        <span className="rounded bg-emerald-500/20 px-2 py-0.5 text-[10px] text-emerald-300 font-mono">СТАРТ</span>
      </button>
