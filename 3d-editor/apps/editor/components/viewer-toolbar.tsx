...
import { NimBlueprintModal } from './nim-blueprint-modal'
...
function Build3dModelButton() {
  const openModal = useNimModal((s) => s.openModal)

  return (
    <ToolbarTooltip label="DeepFloorPlan & FloorPlan3D — Распознавание 2D чертежей и генерация планировок">
      <button
        className="flex items-center gap-1.5 px-3 py-1 font-bold text-white text-xs bg-emerald-600 hover:bg-emerald-500 rounded-xl transition-all shadow-md shadow-emerald-600/30 cursor-pointer border border-emerald-400/40 active:scale-95"
        onClick={openModal}
        type="button"
      >
        <Wand2 className="h-4 w-4 shrink-0 text-white animate-pulse" />
        <span>Студия 2D➔3D</span>
      </button>
    </ToolbarTooltip>
  )
}

function NimAiButton() {
  const openModal = useNimModal((s) => s.openModal)

  return (
    <ToolbarTooltip label="DeepFloorPlan (AI 2D➔3D) & Студия Планировок">
      <button
        className="flex items-center gap-1.5 px-2.5 font-bold text-emerald-400 text-xs transition-colors hover:bg-emerald-500/15 hover:text-emerald-300 border border-emerald-500/30 rounded-lg py-1 bg-emerald-500/10 cursor-pointer shadow-sm"
        onClick={openModal}
        type="button"
      >
        <Sparkles className="h-3.5 w-3.5 shrink-0 text-emerald-400" />
        <span>DeepFloorPlan</span>
      </button>
    </ToolbarTooltip>
  )
}
